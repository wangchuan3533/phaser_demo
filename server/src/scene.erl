...
handle_call(#joinroomreq{room_id = _RoomId}, {Pid, _Tag}, State = #state{elapsed = Elapsed, tile_map = TileMap, player_id_seed = PlayerId, players = Players}) ->
...
handle_call(Checkpoint = #checkpointreq{id = Id, latency = Latency}, {Pid, _Tag}, State = #state{elapsed = Elapsed, players = Players}) ->
...
handle_info(tick, State = #state{tile_map = TileMap, timer = OldTimer, players = Players, elapsed = Elapsed, tick = Tick, start_time = StartTime}) ->
  ...
  Players1 = lists:map(fun(Player = #player_state{entity = Entity, checkpoints = Checkpoints, latency = Latency}) ->
    case queue:peek(Checkpoints) of
      {value, Checkpoint = #checkpointreq{elapsed = ClientElapsed}} when Elapsed - ClientElapsed >= Latency ->
